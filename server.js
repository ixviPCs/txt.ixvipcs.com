...
io.on("connection", (socket) => {
  ...
  socket.on("join", (request, acknowledge) => {
    ...
    if (!account && request?.mode !== "sign-in") { const name = guestName(); if (request.pin && !validPin(request.pin)) return acknowledge?.({ ok: false, error: "A PIN must be 4–6 letters or numbers." }); account = { id: randomUUID(), name, nickname: "", bio: "", avatar: validAvatar(request.avatar), pinHash: request.pin ? hashPin(request.pin) : "", admin: ADMIN_PINS.has(request.pin), createdAt: Date.now() }; data.accounts[account.id] = account; data.devices[deviceId] = account.id; }
